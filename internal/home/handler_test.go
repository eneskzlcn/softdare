...
//	mockSession := mocks.NewMockSessionProvider(controller)
...
//		handler := home.NewHandler(nil, mockRenderer, mockSession, mockHomeService)
//		assert.Nil(t, handler)
//	})
//	t.Run("given nil renderer then it should return nil when NewHandler called", func(t *testing.T) {
//		handler := home.NewHandler(zap.NewExample().Sugar(), nil, mockSession, mockHomeService)
//		assert.Nil(t, handler)
//	})
...
//		handler := home.NewHandler(zap.NewExample().Sugar(), mockRenderer, mockSession, nil)
//		assert.Nil(t, handler)
//	})
//	t.Run("given valid args then it should return handler when newHandler called", func(t *testing.T) {
//		handler := home.NewHandler(zap.NewExample().Sugar(), mockRenderer, mockSession, mockHomeService)
...
//	handler := home.NewHandler(logger, mockRenderer, mockSession, nil)
...
//		Username string
//	}
//	mockSession.EXPECT().Exists(gomock.Any(), gomock.Any()).Return(true)
//	mockSession.EXPECT().Get(gomock.Any(), "user").Return(UserSessionData{Email: "a@s.com", Username: "asdad"})
//
